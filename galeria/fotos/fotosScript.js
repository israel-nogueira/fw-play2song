	//----------------------------------------------------------
	//	DEFININDO A CLASSE fotosScript
	//----------------------------------------------------------
	var fotosScript = function (url, params) {
		return {
			url: url,
			params: params,
			...
			reload: async function () {
				...
				$(document) 
				.off('click','#upload-img-galerias')
				.on('click','#upload-img-galerias', function (e) { 
					e.preventDefault();
					input_1.click();
					return false;
				});
				...
				var input_1 = formUpload.setInput('input_1');
					input_1.multiple(true);
					input_1.accept('.jpg,.jpeg,.png,image/*');
					input_1.on('change', function () {

						ajax({
							csrfKey: 'formUpload',
							url: '/api/galeria/fotos/upload/' + window.location.pathname.split('/')[3] + '/' + window.location.pathname.split('/')[4],
							method: 'POST',
							recaptcha: true,
							uploadAsync: false,
							fila: true,
							...
							uploadForm:formUpload.form(),
							uploadBlockZeroFiles:function () {},
							beforeUpload:function (lista) {},
							uploadOnProgress:function (index,pct) {},
							uploadOnAbort:function (param) {},
							uploadMaxSize:function (param) {},
							uploadOnError:function (param) {},
							uploadOnLoad:function (param) {},
							uploadOnLoadend:function (param) {},
							uploadOnLoadstart:function (param) {},
							uploadOnTimeout:function (param) {},
						});
				...
				$(document)
					.off('click', '.detalhes-thumb')
					.on('click', '.detalhes-thumb', function (e) {
						e.preventDefault();
						var ID = $(this).data('uidthumb')

						ajax({
							csrfKey: 'officeCRSF',
							url: '/api/galeria/fotos/detalhes/' + ID,
							method: 'POST',
							recaptcha: false,
							fila: true,
							beforeSend: function (xhr) { },
							success: async function (response, status, xhr) {
								...
								$('#modal_thumb input[name="TITULO"]').val(response.response.TITULO)
								$('#modal_thumb input[name="DESCRICAO"]').val(response.response.DESCRICAO)
								$('#modal_thumb .close-button').off('click').on('click', async function () { $('#modal_thumb').modal('hide'); })
								$('#modal_thumb .confirm-button').off('click').on('click', async function () {
									$('#modal_thumb').modal('hide');
									await ajax({
										csrfKey: 'officeCRSF',
										url: '/api/galeria/fotos/salvar/' + ID,
										method: 'POST',
										private: $('#form-detalhes-thumb').object(),
										recaptcha: true,
										fila: true,
										beforeSend: function (xhr) { },
										success: function (response, status, xhr) {
				...
				$(document).on('click', '.add-nova-galeria', function (e) {
					...
					ajax({
						csrfKey: 'officeCRSF',
						url: '/api/galeria/fotos/novo/' + window.location.pathname.split('/')[3],
						method: 'POST',
						recaptcha: true,
						fila: true,
						beforeSend: function (xhr) { },
				...
				$(document).on('click', '.delete-thumb', function (e) {
					...
					window.confirm('Você quer excluir mesmo essa galeria?', 'Essa ação é irreversível e irá excluir todas as galerias e fotos contidas dentro dela.', function () {
						ajax({
							csrfKey: 'officeCRSF',
							url: '/api/galeria/fotos/excluir/' + ID,
							method: 'POST',
							recaptcha: true,
							uploadAsync: true,
							fila: true,
							...
							success: function (response, status, xhr) {
								...
								setTimeout(function(){params.startDT.ajax.reload();},1000)
							},
							error: function (errorMsg, xhr) { window.errorAjax('Ops, houve uma falha', 'Não sei dizer aonde foi') },
							done: function (xhr) { },
			...
			initTable: async function () {
				...
				const ESTILO_LISTA = [
					...
					{
						'template': '#' + prefix + '_lista_thumb',
						'container': '<div class="' + prefix + '_vitrine row"></div>',
					},
				...
				*/
				...
				const colunas = [
					...
					{ data: 'UID_GALERIA', name: 'UID_GALERIA', sTitle: "UID_GALERIA", className: '', width: "auto", orderable: true, sType: "string", visible: true, fnCreatedCell: function (aData, nTd, sData, oData, iRow, iCol) { }, },
					{ data: 'UID_PLUGIN', name: 'UID_PLUGIN', sTitle: "UID_GALERIA", className: '', width: "auto", orderable: true, sType: "string", visible: true, fnCreatedCell: function (aData, nTd, sData, oData, iRow, iCol) { }, },
					{ data: 'FILENAME', name: 'FILENAME', sTitle: "FILENAME", className: '', width: "auto", orderable: true, sType: "string", visible: true, fnCreatedCell: function (aData, nTd, sData, oData, iRow, iCol) { }, },
					{ data: 'ORIGINAL_NAME', name: 'ORIGINAL_NAME', sTitle: "ORIGINAL_NAME", className: '', width: "auto", orderable: true, sType: "string", visible: true, fnCreatedCell: function (aData, nTd, sData, oData, iRow, iCol) { }, },
					{ data: '100x100', name: '100x100', sTitle: "100x100", className: '', width: "auto", orderable: true, sType: "string", visible: true, fnCreatedCell: function (aData, nTd, sData, oData, iRow, iCol) { }, },
				];
				...
				*/
				...
				params.startDT = $(idDT).DataTable({
					...
					'columns': colunas,
					rowCallback: function (row, data) {montaVitrine(row, data)},
					...
					"ajax": function (request, drawCallback, settings) {
						...
						ajax({
							url: '/api/galeria/fotos/listar/' + window.location.pathname.split('/')[3] + '/' + window.location.pathname.split('/')[4],
							csrfKey: 'officeCRSF',
							method: 'POST',
