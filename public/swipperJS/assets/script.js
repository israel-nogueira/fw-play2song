(function () {
	...
	new MutationObserver((r) => {
		for (const n of r) if (n.type === "childList") for (const l of n.addedNodes) l.tagName === "LINK" && l.rel === "modulepreload" && s(l);
...
function Ee(t, e) {
	...
	return (
		i.WebKitCSSMatrix
			...
			: ((n = l.MozTransform || l.OTransform || l.MsTransform || l.msTransform || l.transform || l.getPropertyValue("transform").replace("translate(", "matrix(1, 0, 0, 1,")), (s = n.toString().split(","))),
...
function Oe(t) {
	...
	return (
		!c && h && i.touch && v.indexOf(`${o}x${a}`) >= 0 && ((c = n.match(/(Version)\/([\d.]+)/)), c || (c = [0, 1, "13_0_0"]), (h = !1)),
...
function De(t) {
	...
	const n = [],
		...
		o = function (c, f) {
			...
			u.observe(c, { attributes: typeof f.attributes > "u" ? !0 : f.attributes, childList: typeof f.childList > "u" ? !0 : f.childList, characterData: typeof f.characterData > "u" ? !0 : f.characterData }), n.push(u);
		},
		a = () => {
	...
	i({ observer: !1, observeParents: !1, observeSlideChildren: !1 }), s("init", a), s("destroy", d);
}
var Ve = {
	on(t, e, i) {
		const s = this;
	...
	emit() {
		...
		return (
			typeof n[0] == "string" || Array.isArray(n[0]) ? ((e = n[0]), (i = n.slice(1, n.length)), (s = t)) : ((e = n[0].events), (i = n[0].data), (s = n[0].context || t)),
			i.unshift(s),
...
function Fe() {
	...
	typeof t.params.width < "u" && t.params.width !== null ? (e = t.params.width) : (e = s.clientWidth),
		...
		!((e === 0 && t.isHorizontal()) || (i === 0 && t.isVertical())) &&
			((e = e - parseInt(F(s, "padding-left") || 0, 10) - parseInt(F(s, "padding-right") || 0, 10)),
...
function Ne() {
	...
	for (let g = 0; g < f; g += 1) {
		...
		if ((c[g] && (E = c[g]), O && t.grid.updateSlide(g, E, c), !(c[g] && F(E, "display") === "none"))) {
			if (i.slidesPerView === "auto") {
				...
				if ((y && (E.style.transform = "none"), I && (E.style.webkitTransform = "none"), i.roundLengths)) L = t.isHorizontal() ? oe(E, "width", !0) : oe(E, "height", !0);
				else {
					const C = e(M, "width"),
						A = e(M, "padding-left"),
						_ = e(M, "padding-right"),
					...
					if (ne && ne === "border-box") L = C + se + re;
					else {
						const { clientWidth: we, offsetWidth: Se } = E;
						L = C + A + _ + se + re + (Se - we);
					}
				}
				y && (E.style.transform = y), I && (E.style.webkitTransform = I), i.roundLengths && (L = Math.floor(L));
			} else (L = (n - (i.slidesPerView - 1) * m) / i.slidesPerView), i.roundLengths && (L = Math.floor(L)), c[g] && (c[g].style[t.getDirectionLabel("width")] = `${L}px`);
			c[g] && (c[g].swiperSlideSize = L),
				...
				i.centeredSlides
					...
					u.push(x))
					: (i.roundLengths && (x = Math.floor(x)), (T - Math.min(t.params.slidesPerGroupSkip, T)) % t.params.slidesPerGroup === 0 && p.push(x), u.push(x), (x = x + L + m)),
				(t.virtualSize += L + m),
				(S = L),
				(T += 1);
		}
	...
	if (
		...
		l && o && (i.effect === "slide" || i.effect === "coverflow") && (s.style.width = `${t.virtualSize + m}px`),
		i.setWrapperSize && (s.style[t.getDirectionLabel("width")] = `${t.virtualSize + m}px`),
		O && t.grid.updateWrapperSize(L, p),
		!i.centeredSlides)
	) {
		const g = [];
		for (let E = 0; E < p.length; E += 1) {
			let M = p[E];
			i.roundLengths && (M = Math.floor(M)), p[E] <= t.virtualSize - n && g.push(M);
		}
		(p = g), Math.floor(t.virtualSize - n) - Math.floor(p[p.length - 1]) > 1 && p.push(t.virtualSize - n);
	}
	if (a && i.loop) {
		const g = h[0] + m;
		...
		for (let E = 0; E < t.virtual.slidesBefore + t.virtual.slidesAfter; E += 1) i.slidesPerGroup === 1 && p.push(p[p.length - 1] + g), u.push(u[u.length - 1] + g), (t.virtualSize += g);
	}
	...
	if ((Object.assign(t, { slides: c, snapGrid: p, slidesGrid: u, slidesSizesGrid: h }), i.centeredSlides && i.cssMode && !i.centeredSlidesBounds)) {
		R(s, "--swiper-centered-offset-before", `${-p[0]}px`), R(s, "--swiper-centered-offset-after", `${t.size / 2 - h[h.length - 1] / 2}px`);
		const g = -t.snapGrid[0],
			E = -t.slidesGrid[0];
		(t.snapGrid = t.snapGrid.map((M) => M + g)), (t.slidesGrid = t.slidesGrid.map((M) => M + E));
	}
	if (
		(f !== d && t.emit("slidesLengthChange"),
		p.length !== w && (t.params.watchOverflow && t.checkOverflow(), t.emit("snapGridLengthChange")),
		u.length !== P && t.emit("slidesGridLengthChange"),
...
function Re(t) {
	...
	if (i.loop) {
		const c = e.getSlideIndexByData(0),
			...
			v = Math.abs(t);
		v >= p ? (o = (v - p) / h) : (o = (v + h - u) / h), o > 1 && (o -= 1);
	}
	Object.assign(e, { progress: r, progressLoop: o, isBeginning: n, isEnd: l }),
...
function He() {
	...
	else l ? ((a = e.filter((f) => f.column === r)[0]), (c = e.filter((f) => f.column === r + 1)[0]), (d = e.filter((f) => f.column === r - 1)[0])) : (a = e[r]);
	a &&
		...
		l
			...
			(d = Me(a, `.${i.slideClass}, swiper-slide`)[0]),
...
const H = (t, e) => {
	...
	te = (t) => {
		...
		else for (let l = Math.max(r - e, 0); l <= Math.min(n + e, i - 1); l += 1) l !== r && (l > n || l < r) && U(t, l);
	};
function We(t) {
...
function je(t) {
	...
	let p;
	if (e.virtual && r.virtual.enabled && r.loop) p = c(a);
	else if (f) {
		const u = e.slides.filter((v) => v.column === a)[0];
		let h = parseInt(u.getAttribute("data-swiper-slide-index"), 10);
		Number.isNaN(h) && (h = Math.max(e.slides.indexOf(u), 0)), (p = Math.floor(h / r.grid.rows));
	} else if (e.slides[a]) {
		const u = e.slides[a].getAttribute("data-swiper-slide-index");
		u ? (p = parseInt(u, 10)) : (p = a);
	} else p = a;
	Object.assign(e, { previousSnapIndex: o, snapIndex: d, previousRealIndex: l, realIndex: p, previousIndex: n, activeIndex: a }),
		e.initialized && te(e),
...
function qe(t, e) {
	...
	if (r && n) (i.clickedSlide = r), i.virtual && i.params.virtual.enabled ? (i.clickedIndex = parseInt(r.getAttribute("data-swiper-slide-index"), 10)) : (i.clickedIndex = l);
	else {
...
var Ye = { updateSize: Fe, updateSlides: Ne, updateAutoHeight: Be, updateSlidesOffset: _e, updateSlidesProgress: $e, updateProgress: Re, updateSlidesClasses: He, updateActiveIndex: je, updateClickedSlide: qe };
function Xe(t) {
...
function Ke(t, e) {
	...
	i.isHorizontal() ? (o = s ? -t : t) : (a = t),
		...
		r.cssMode
			? (n[i.isHorizontal() ? "scrollLeft" : "scrollTop"] = i.isHorizontal() ? -o : -a)
			: r.virtualTranslate || (i.isHorizontal() ? (o -= i.cssOverflowAdjustment()) : (a -= i.cssOverflowAdjustment()), (n.style.transform = `translate3d(${o}px, ${a}px, ${d}px)`));
	let c;
...
function Qe(t, e, i, s, r) {
	...
	if ((s && t > a ? (c = a) : s && t < d ? (c = d) : (c = t), n.updateProgress(c), l.cssMode)) {
		...
		if (e === 0) o[f ? "scrollLeft" : "scrollTop"] = -c;
			...
			if (!n.support.smoothScroll) return me({ swiper: n, targetPosition: -c, side: f ? "left" : "top" }), !0;
...
var Ze = { getTranslate: Xe, setTranslate: Ke, minTranslate: Ue, maxTranslate: Je, translateTo: Qe };
function et(t, e) {
	const i = this;
...
function tt(t, e) {
	...
	const i = this,
		{ params: s } = i;
	s.cssMode || (s.autoHeight && i.updateAutoHeight(), ge({ swiper: i, runCallbacks: t, direction: e, step: "Start" }));
}
function it(t, e) {
	t === void 0 && (t = !0);
	const i = this,
		{ params: s } = i;
	(i.animating = !1), !s.cssMode && (i.setTransition(0), ge({ swiper: i, runCallbacks: t, direction: e, step: "End" }));
}
...
function rt(t, e, i, s, r) {
	...
	if (o.cssMode) {
		...
		if (e === 0) {
			...
			if (!n.support.smoothScroll) return me({ swiper: n, targetPosition: x, side: m ? "left" : "top" }), !0;
		...
		e === 0
			...
			n.onSlideToWrapperTransitionEnd ||
				(n.onSlideToWrapperTransitionEnd = function (x) {
					!n ||
						n.destroyed ||
						(x.target === this && (n.wrapperEl.removeEventListener("transitionend", n.onSlideToWrapperTransitionEnd), (n.onSlideToWrapperTransitionEnd = null), delete n.onSlideToWrapperTransitionEnd, n.transitionEnd(i, P)));
				}),
...
function nt(t, e, i, s) {
	...
	if (r.params.loop)
		if (r.virtual && r.params.virtual.enabled) l = l + r.virtual.slidesBefore;
			...
			} else o = r.getSlideIndexByData(l);
			...
			if ((d && (f = f || o < Math.ceil(c / 2)), f)) {
				...
				r.loopFix({ direction: p, slideTo: !0, activeSlideIndex: p === "next" ? o + 1 : o - a + 1, slideRealIndex: p === "next" ? r.realIndex : void 0 });
			}
			if (n) {
				const p = l * r.params.grid.rows;
...
function lt(t, e, i) {
	...
	let b = 0;
	if (
		(typeof v < "u" && ((b = l.indexOf(v)), b < 0 && (b = s.activeIndex - 1), r.slidesPerView === "auto" && r.slidesPerGroup === 1 && r.slidesPerGroupAuto && ((b = b - s.slidesPerViewDynamic("previous", !0) + 1), (b = Math.max(b, 0)))),
		...
		(n = parseInt(t.clickedSlide.getAttribute("data-swiper-slide-index"), 10)),
			e.centeredSlides
				? r < t.loopedSlides - s / 2 || r > t.slides.length - t.loopedSlides + s / 2
					? (t.loopFix(),
					(r = t.getSlideIndex(V(i, `${l}[data-swiper-slide-index="${n}"]`)[0])),
					Z(() => {
						t.slideTo(r);
					}))
					: t.slideTo(r)
...
var ft = { slideTo: rt, slideToLoop: nt, slideNext: at, slidePrev: lt, slideReset: ot, slideToClosest: dt, slideToClickedSlide: ct };
function ut(t) {
	const e = this,
...
function pt(t) {
	...
	let w = b;
	w % b !== 0 && (w += b - (w % b)), (w += u.loopAdditionalSlides), (a.loopedSlides = w);
	const P = a.grid && u.grid && u.grid.rows > 1;
	d.length < v + w
		? j(
			"Swiper Loop Warning: The number of slides is not enough for loop mode, it will be disabled and not function properly. You need to add more slides (or make duplicates) or lower the values of slidesPerView and slidesPerGroup parameters"
		)
		: P && u.grid.fill === "row" && j("Swiper Loop Warning: Loop mode is not compatible with grid.fill = `row`");
	const m = [],
		x = [];
	let S = a.activeIndex;
	...
	const g = P ? Math.ceil(d.length / u.grid.rows) : d.length,
		M = (P ? d[n].column : n) + (h && typeof r > "u" ? -v / 2 + 0.5 : 0);
	if (M < w) {
		L = Math.max(w - M, b);
		for (let y = 0; y < w - M; y += 1) {
			...
			if (P) {
				const C = g - I - 1;
				for (let A = d.length - 1; A >= 0; A -= 1) d[A].column === C && m.push(A);
			} else m.push(g - I - 1);
		}
	} else if (M + v > g - w) {
	...
	if (
		...
		u.slidesPerView === "auto"
			? a.updateSlides()
			: P &&
			((m.length > 0 && O) || (x.length > 0 && T)) &&
			a.slides.forEach((y, I) => {
		...
		if (m.length > 0 && O) {
			if (typeof e > "u") {
				...
				o
					...
					: (a.slideTo(S + L, 0, !1, !0), r && ((a.touchEventsData.startTranslate = a.touchEventsData.startTranslate - C), (a.touchEventsData.currentTranslate = a.touchEventsData.currentTranslate - C)));
			} else if (r) {
			...
			if (typeof e > "u") {
				...
				o
					? a.setTranslate(a.translate - C)
					: (a.slideTo(S - z, 0, !1, !0), r && ((a.touchEventsData.startTranslate = a.touchEventsData.startTranslate - C), (a.touchEventsData.currentTranslate = a.touchEventsData.currentTranslate - C)));
			} else {
	...
	if (((a.allowSlidePrev = c), (a.allowSlideNext = f), a.controller && a.controller.control && !l)) {
		const y = { slideRealIndex: e, direction: s, setTranslate: r, activeSlideIndex: n, byController: !0 };
		Array.isArray(a.controller.control)
			? a.controller.control.forEach((I) => {
				!I.destroyed && I.params.loop && I.loopFix({ ...y, slideTo: I.params.slidesPerView === u.slidesPerView ? i : !1 });
			})
			: a.controller.control instanceof a.constructor && a.controller.control.params.loop && a.controller.control.loopFix({ ...y, slideTo: a.controller.control.params.slidesPerView === u.slidesPerView ? i : !1 });
	}
	a.emit("loopFix");
}
function mt() {
...
function Tt(t) {
	...
	if (!o || (!n.simulateTouch && s.pointerType === "mouse") || (e.animating && n.preventInteractionOnTransition)) return;
	!e.animating && n.cssMode && n.loop && e.loopFix();
	let a = s.target;
	if ((n.touchEventsTarget === "wrapper" && !e.wrapperEl.contains(a)) || ("which" in s && s.which === 3) || ("button" in s && s.button > 0) || (r.isTouched && r.isMoved)) return;
...
function xt(t) {
	...
	let d;
	if (a.type === "touchmove") {
		if (((d = [...a.changedTouches].filter((T) => T.identifier === s.touchId)[0]), !d || d.identifier !== s.touchId)) return;
	} else d = a;
	if (!s.isTouched) {
	...
	if (!i.allowTouchMove) {
		a.target.matches(s.focusableElements) || (i.allowClick = !1), s.isTouched && (Object.assign(n, { startX: c, startY: f, currentX: c, currentY: f }), (s.touchStartTime = W()));
		return;
	}
	...
	if (typeof s.isScrolling > "u") {
		...
		(i.isHorizontal() && n.currentY === n.startY) || (i.isVertical() && n.currentX === n.startX)
			? (s.isScrolling = !1)
			: p * p + u * u >= 25 && ((T = (Math.atan2(Math.abs(u), Math.abs(p)) * 180) / Math.PI), (s.isScrolling = i.isHorizontal() ? T > r.touchAngle : 90 - T > r.touchAngle));
	}
	...
	if (!s.isMoved) {
		if ((w && P && i.loopFix({ direction: i.swipeDirection }), (s.startTranslate = i.getTranslate()), i.setTransition(0), i.animating)) {
	...
	if ((new Date().getTime(), s.isMoved && s.allowThresholdMove && b !== i.touchesDirection && w && P && Math.abs(h) >= 1)) {
		Object.assign(n, { startX: c, startY: f, currentX: c, currentY: f, startTranslate: s.currentTranslate }), (s.loopSwapReset = !0), (s.startTranslate = s.currentTranslate);
		return;
	}
	...
	if (
		...
		h > 0
			...
			s.currentTranslate > i.minTranslate() && ((x = !1), r.resistance && (s.currentTranslate = i.minTranslate() - 1 + (-i.minTranslate() + s.startTranslate + h) ** S)))
			: h < 0 &&
			(w &&
				P &&
				!m &&
				...
				i.loopFix({ direction: "next", setTranslate: !0, activeSlideIndex: i.slides.length - (r.slidesPerView === "auto" ? i.slidesPerViewDynamic() : Math.ceil(parseFloat(r.slidesPerView, 10))) }),
			s.currentTranslate < i.maxTranslate() && ((x = !1), r.resistance && (s.currentTranslate = i.maxTranslate() + 1 - (i.maxTranslate() - s.startTranslate - h) ** S))),
...
function yt(t) {
	...
	if (s.type === "touchend" || s.type === "touchcancel") {
		if (((r = [...s.changedTouches].filter((S) => S.identifier === i.touchId)[0]), !r || r.identifier !== i.touchId)) return;
	} else {
	...
	if (e.allowClick) {
		...
		e.updateClickedSlide((S && S[0]) || s.target, S), e.emit("tap click", s), p < 300 && f - i.lastClickTime < 300 && e.emit("doubleTap doubleClick", s);
	}
	if (
		((i.lastClickTime = W()),
		Z(() => {
	...
	if (p > l.longSwipesMs) {
		...
		e.navigation && (s.target === e.navigation.nextEl || s.target === e.navigation.prevEl)
			? s.target === e.navigation.nextEl
				? e.slideTo(v + x)
				: e.slideTo(v)
			: (e.swipeDirection === "next" && e.slideTo(w !== null ? w : v + x), e.swipeDirection === "prev" && e.slideTo(P !== null ? P : v));
	}
}
function ce() {
	const t = this,
...
function Pt(t) {
	const e = this;
	H(e, t.target), !(e.params.cssMode || (e.params.slidesPerView !== "auto" && !e.params.autoHeight)) && e.update();
}
function Mt() {
	const t = this;
...
function It() {
	const t = this,
		{ params: e } = t;
	(t.onTouchStart = Tt.bind(t)), (t.onTouchMove = xt.bind(t)), (t.onTouchEnd = yt.bind(t)), (t.onDocumentTouchStart = Mt.bind(t)), e.cssMode && (t.onScroll = Et.bind(t)), (t.onClick = bt.bind(t)), (t.onLoad = Pt.bind(t)), ve(t, "on");
}
function Ct() {
	ve(this, "off");
}
	...
	d && !c
		...
		["navigation", "pagination", "scrollbar"].forEach((w) => {
			...
			const P = s[w] && s[w].enabled,
				m = a[w] && a[w].enabled;
			P && !m && t[w].disable(), !P && m && t[w].enable();
		});
...
function Bt(t, e) {
	return function (s) {
		...
		typeof t[r] == "object" && !("enabled" in t[r]) && (t[r].enabled = !0), t[r] || (t[r] = { enabled: !1 }), G(e, s);
	};
}
const J = { eventsEmitter: Ve, update: Ye, translate: Ze, transition: st, slide: ft, loop: ht, grabCursor: wt, events: Lt, breakpoints: At, checkOverflow: Nt, classes: Vt },
	Q = {};
class D {
	constructor() {
		let e, i;
		for (var s = arguments.length, r = new Array(s), n = 0; n < s; n++) r[n] = arguments[n];
		r.length === 1 && r[0].constructor && Object.prototype.toString.call(r[0]).slice(8, -1) === "Object" ? (i = r[0]) : ([e, i] = r), i || (i = {}), (i = G({}, i)), e && !i.el && (i.el = e);
		const l = N();
		...
		const a = {};
		o.modules.forEach((c) => {
			c({ params: i, swiper: o, extendParams: Bt(i, a), on: o.on.bind(o), once: o.once.bind(o), off: o.off.bind(o), emit: o.emit.bind(o) });
		});
		const d = G({}, ue, a);
		return (
			(o.params = G({}, d, Q, i)),
	...
	slidesPerViewDynamic(e, i) {
		...
		if (r.centeredSlides) {
			let f = n[d] ? n[d].swiperSlideSize : 0,
				p;
			for (let u = d + 1; u < n.length; u += 1) n[u] && !p && ((f += n[u].swiperSlideSize), (c += 1), f > a && (p = !0));
			for (let u = d - 1; u >= 0; u -= 1) n[u] && !p && ((f += n[u].swiperSlideSize), (c += 1), f > a && (p = !0));
		} else if (e === "current") for (let f = d + 1; f < n.length; f += 1) (i ? l[f] + o[f] - l[d] < a : l[f] - l[d] < a) && (c += 1);
	...
	changeLanguageDirection(e) {
		...
		(i.rtl && e === "rtl") ||
			...
			i.rtl ? (i.el.classList.add(`${i.params.containerModifierClass}rtl`), (i.el.dir = "rtl")) : (i.el.classList.remove(`${i.params.containerModifierClass}rtl`), (i.el.dir = "ltr")),
			i.update());
	}
	mount(e) {
		const i = this;
		...
		return (
			!l &&
				...
				((l = ee("div", i.params.wrapperClass)),
				s.append(l),
...
function Rt({ swiper: t, on: e }) {
	const i = () => {
				...
				if (m <= 0)
					if (f && a > 1) {
						if ((m <= 0 && m >= -(a - 2) && ((O = S), (T = 1), (L = 1)), M && m < -(a - Math.ceil(a / 2)))) {
							...
							(T = I), (L = T ** 4), (O = S + z * (1 - I) * (1 + g * 2));
						}
						if (y && m < -(a / 2 - 1) && m >= -(a / 2)) {
							const I = a / 2 - Math.abs(m);
							(T = p - g + (u + g * 2) * (a / 2 - Math.abs(m))), (L = ((T - p) / (1 - p)) ** 4), (O = S + z * (u + g) * (1 - I));
						}
						if (y && m < -a / 2) {
							let I = a / 2 + 1 - Math.abs(m);
							(T = 0), I >= 0 && ((I = -g * 2 + I * (1 + g * 2)), (I = Math.max(Math.min(I, 1), 0)), (T = (u - g) * I)), (O = S + z * (u + g) * (2 - I) + z * (u - g) * (1 - I));
						}
					} else (T = 1 + m), (O = -x), (L = T ** 4);
				if (a === 1) m > 0 && ((T = 1 - m), (O = -x + o * Math.min(m, 1)), (L = T ** 4));
				else {
					if ((m > 0 && m <= a - 2 && ((O = S), (T = 1), (L = 1)), y ? m > a / 2 - 1 && m <= a / 2 : m > a - 2 && m <= a - 1)) {
						const C = y ? Math.floor(a / 2) : 1;
						(T = p - g + (u + g * 2) * (a - C - Math.abs(m))), (O = S), (L = ((T - p) / (1 - p)) ** 4);
					}
					if (M && m > a - Math.ceil(a / 2)) {
						const C = Math.ceil(a / 2) - (a - Math.abs(m));
						(O = S - z * (g * 2) * C), (T = 1 - C), (L = T ** 4);
					}
					if (m > a - 1 && m <= a && !f) {
						const C = a - Math.abs(m),
							A = p - g,
							_ = u - g;
						(T = _ + (A - _) * C), (O = S - z * (u + g) * (1 - C));
					}
					if (m > (f ? a / 2 : a) && !M) {
						let C = (f ? a / 2 + 1 : a + 1) - Math.abs(m),
							A = 0;
						(T = 0), C >= 0 && ((C = -g * 2 + C * (1 + g * 2)), (C = Math.max(Math.min(C, 1), 0)), (T = (u - g) * C), (A = -C * (u + g) * z + C * d)), (O = -x + o * Math.min(m, 1) + A), (L = 0);
					}
				}
				T < 0 && (T = 0),
					T > 1 && (T = 1),
					...
					P.forEach((I) => {
						...
						(Number.isNaN(C) || (!C && C !== 0)) && (C = 1), (I.style.transform = `scale(${1 + (C - 1) * (1 - T)})`);
					}),
		...
		s = (r) => {
			...
			$t({ swiper: t, duration: r, transformElements: l, allSlides: !0 });
		};
	e("beforeInit", () => {
		...
		const r = { loopAdditionalSlides: 1, watchSlidesProgress: !0, virtualTranslate: !t.params.cssMode };
...
function customAnimation(options) {
	...
	function animate(timestamp) {
		...
		var progressPercentage = Math.min(progress / options.duration, 1);
		options.onProgress && options.onProgress(progressPercentage);
...
window.q.on('slideChange', function () {
	/* AQUI É O ELEMENTO DA DIV ATIVA */
	var elemento_active = window.q.slides[window.q.activeIndex];
	AutoPlay()

	var titulo		= elemento_active.getAttribute('data-title');
	var description	= elemento_active.getAttribute('data-description');
	var textos	= document.querySelector(".div-conteudo .textos");
	textos.innerHTML = "<h1>" + titulo + "</h1><p>" + description +"</p>"
	console.log(elemento_active,titulo,description)
});
...
function AutoPlay(){
	...
	animation = customAnimation({
		...
		onProgress: function (progress) {
			/* AQUI É O ELEMENTO ATIVO DA ARRAY GERAL */
			window.q.slides[window.q.activeIndex].querySelector(".autoplay-progress svg").style.setProperty("--progress", progress);
		},
		onFinish: function () {
			window.q.slideNext()
			AutoPlay();
